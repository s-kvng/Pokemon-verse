...
const types = [
    ...
    {name : "grass" },{name : "electric" },
    {name : "psychic" },{name : "ice" },
    {name : "dragon" },{name : "dark" },
    {name : "fairy" },
]
...
const Main = () => {
  return (
    <section className=" h-[75vh] md:h-[72vh] max-[72vh] min-[72vh] rounded-[8px] bg-navGrad overflow-y-scroll mb-5 md:mb-2">
        ...
        <label for="type-selection" className="text-zinc-500 me-2">Choose the type of pokemon</label>
        <select name="types" id="type-selection" 
        className="focus:ring-0 outline-0 rounded-[8px] p-1 w-[50%] shadow-lg cursor-pointer text-center">
           
           {types.map((type , index)=>(
             <option className=" even:bg-white placeholder:italic" key={index}  value={type.name}>{type.name}</option>
           ))}
